...
后续复查见 [上游兼容性与生产程序验证](COMPATIBILITY_20260909.md)。MS 输入/输出也接受 `.MS`。
默认时间范围只裁掉两端全标记时段，保留中间全标记时段及其后续有效数据；逐样本标记仍有效。
校准解 FITS 的平均时间同样保留实际 21CMA 时间质心。应用多时间块解时优先使用完整的逐块
起止时间/质心；缺失逐块时间的 AO 文件按输入观测的时间跨度等分匹配，不能恢复不规则观测
原有的解块划分，因此应保留并使用原始 FITS。
...
- CPU/CUDA 都按既有 GPU 限制拒绝 `|alpha|` 或 `|beta| > 1e-3`、增益不在 `0..=1.5` 的拟合。
  拒绝时沿用之前通过的参数，日志会给出源、时间块和原因；JSON 中的沿用值不表示求解成功。
- `--iono-time-average 3` 按顺序组合三个读入时间块；最后不足三个也可处理。跨很长间隔组合数据会假设电离层参数在这些采样之间不变，应根据科学需求选择时间区间。
- `--iono-freq-average 7` 可处理不能被 7 整除的频道数，无需裁掉最后几个频道。最后一块使用实际包含频道的中心频率。CPU/CUDA 每轮都先在原始采样上更新电离层模型再平均。
