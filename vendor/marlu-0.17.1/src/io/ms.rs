...
impl MeasurementSetWriter {
    ...
    pub fn add_cotter_mods(&self, num_channels: usize) -> Result<(), MeasurementSetWriteError> {
        self.add_cotter_mods_with_pols(num_channels, 4)
    }

    /// Add the visibility columns with an explicit number of correlations.
    pub fn add_cotter_mods_with_pols(
        &self,
        num_channels: usize,
        num_pols: usize,
    ) -> Result<(), MeasurementSetWriteError> {
        assert!((1..=4).contains(&num_pols));
        let comment =
        ...
        let data_shape = [num_channels as _, num_pols as _];
    ...
    pub fn initialize(
        ...
        self.add_cotter_mods_with_pols(num_avg_chans, vis_ctx.num_vis_pols)?;
        ...
        let (corr_product, corr_type) = match vis_ctx.num_vis_pols {
            1 => (array![[0, 0]], vec![9]),
            2 => (array![[0, 0], [1, 1]], vec![9, 12]),
            3 => (array![[0, 0], [0, 1], [1, 1]], vec![9, 10, 12]),
            4 => (
                array![[0, 0], [0, 1], [1, 0], [1, 1]],
                vec![9, 10, 11, 12],
            ),
            n => panic!("Unsupported number of visibility correlations: {n}"),
        };
    ...
    pub fn write_main_row(
        ...
        let num_pols = data.len_of(Axis(1));
            ...
            for ((ant1_idx, ant2_idx), vis_chunk, weight_chunk) in izip!(
                ...
                for (
                    ...
                    if vis_ctx.trivial_averaging() {
                        jones_tmp_view.assign(&ArrayView::from(vis_chunk[[0, 0]].as_slice()));
                    } else {
                ...
                for (mut data, jones) in data_tmp.outer_iter_mut().zip(jones_tmp.outer_iter()) {
                    match num_vis_pols {
                        1 => data[0] = jones[0],
                        2 => {
                            data[0] = jones[0];
